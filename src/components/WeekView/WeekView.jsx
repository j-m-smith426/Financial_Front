...
const WeekView = (props) => {
    ...
    const { data = [], error } = useGetBillsByDateQuery([formatDate(weekRange[0]), formatDate(add(weekRange[0], { days:6}))
    ]);
    ...
    function countBills(date) {
        ...
        for (let i = 0; i < result.length; i++){
            ...
            if (date.getDate() == resultDate) {
                ...
                return (
                    <>
                        {result[i].length}  Bills, <strong>Total</strong>: ${sum}
                </>
            )
        }
        }
      ...
      <div>
       {weekRange.map((date) => (
           ...
           <DayBlock key={date.getDate()}>
               <div onClick={() => handleDayClick(date)} className="day_block">
